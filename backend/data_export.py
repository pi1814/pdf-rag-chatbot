...
CHROMA_PATH = "chroma"
DATA_PATH = "data"
MAX_BATCH_SIZE = 166  # Maximum allowed batch size for Chroma

def main():
    # Check if the database should be cleared (using the --clear flag).
    parser = argparse.ArgumentParser()
    parser.add_argument("--reset", action="store_true", help="Reset the database.")
    parser.add_argument("--model", action="model", help="model to use for embeddings")
...
def add_to_chroma(chunks: list[Document], model="mistral"):
    ...
    if len(new_chunks):
        ...
        for i in range(0, len(new_chunks), MAX_BATCH_SIZE):
            ...
            db.persist()  # Persist after each batch
